...
| `otp_field.dart` | The boxed code entry. Deliberately **one** `TextField` holding the whole code with the boxes drawn as decoration — six separate fields cannot handle a pasted or autofilled code, because a per-box `maxLength: 1` truncates it. |
